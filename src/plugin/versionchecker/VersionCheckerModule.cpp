...
namespace UKControllerPlugin::VersionChecker {
    ...
    void VersionCheckerModule::BootstrapPlugin(Bootstrap::PersistenceContainer &container) {
        EventBus::Bus()
            .AddHandler<UpdateRequiredEvent>(
                std::make_shared<SendUpdateRequiredUserMessage>(*container.plugin),
                EventHandlerFlags::EuroscopeThread);
    ...
    void VersionCheckerModule::Check(VersionInterface &&iface, const char *label) {
        ...
        if (
            (current = GetVersion([&iface]() { return iface.GetCurrentVersion(); }, "current")).empty() ||
