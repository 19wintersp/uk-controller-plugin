    ...
    namespace Hold {
        ...
        class HoldSelectionMenuTest : public Test
        {
                  ...
                  holdManager(mockApi, mockTaskRunner), holdSelectionMenu(holdManager, mockPlugin, 1)
            {
                ...
                ON_CALL(this->mockUserSettingProvider, GetKey(_)).WillByDefault(Return(""));
            }
        ...
        {
            std::shared_ptr<HoldDisplayManager> displayManager = std::make_shared<HoldDisplayManager>(displayFactory);
        ...
        {
            std::shared_ptr<HoldDisplayManager> displayManager1 = std::make_shared<HoldDisplayManager>(displayFactory);

            std::shared_ptr<HoldDisplayManager> displayManager2 = std::make_shared<HoldDisplayManager>(displayFactory);
            ...
            EXPECT_CALL(this->mockPlugin, TriggerPopupList(RectEq(expectedArea), "Hold", 1)).Times(1);

            EXPECT_CALL(this->mockPlugin, AddItemToPopupList(menuItemHold1)).Times(1);

            // WILLO is common to both managers, but should only appear once
            EXPECT_CALL(this->mockPlugin, AddItemToPopupList(menuItemHold2)).Times(1);

            EXPECT_CALL(this->mockPlugin, AddItemToPopupList(menuItemHold3)).Times(1);

            EXPECT_CALL(this->mockPlugin, AddItemToPopupList(menuItemCancel)).Times(1);
        ...
        TEST_F(HoldSelectionMenuTest, ItDisplaysEmptyMenuIfNoHolds)
        {
            ...
            this->holdSelectionMenu.DisplayMenu(*this->mockFlightplan, *this->mockRadarTarget, "test", {0, 0});
        }

        TEST_F(HoldSelectionMenuTest, SelectingAMenuItemAddsAircraftToCorrespondingHold)
        {
            ON_CALL(this->mockPlugin, GetSelectedFlightplan()).WillByDefault(Return(this->mockFlightplan));

            ON_CALL(*this->mockFlightplan, IsTrackedByUser()).WillByDefault(Return(true));

            EXPECT_CALL(this->mockApi, AssignAircraftToHold("BAW123", "WILLO")).Times(1);
        ...
        TEST_F(HoldSelectionMenuTest, SelectingAMenuItemDoesntDoAnythingIfAlreadyAssignedToSpecifiedHold)
        {
            ...
            EXPECT_CALL(this->mockApi, AssignAircraftToHold("BAW123", "WILLO")).Times(0);
        ...
        TEST_F(HoldSelectionMenuTest, ClickingTheDashedMenuItemRemovesAircraftFromHold)
        {
            ...
            EXPECT_CALL(this->mockApi, UnassignAircraftHold("BAW123")).Times(1);
        ...
        TEST_F(HoldSelectionMenuTest, ClickingTheDashedMenuItemDoesNothingIfNotAssignedHold)
        {
            ...
            EXPECT_CALL(this->mockApi, UnassignAircraftHold("BAW123")).Times(0);
